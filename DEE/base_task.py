...
class TaskSetting(object):
    ...
    base_key_attrs = ['data_dir', 'model_dir', 'output_dir', 'resume_latest_cpt', 'event_type_weight', 'start_epoch', 'num_set_decoder_layers', 'num_role_decoder_layers',
                      'num_generated_sets', 'use_pgd', 'cost_weight', 'train_on_multi_events', 'event_type_classes','train_on_multi_roles', 'train_on_single_event',
                       'use_event_type_enc', 'use_role_decoder', 'use_sent_span_encoder']
...
class BasePytorchTask(object):
    ...
    def base_train(self, get_loss_func, eval_function = None, kwargs_dict1={},
    ...
    def base_eval(self, eval_dataset, get_info_on_batch, load_model = False, reduce_info_type='mean', dump_pkl_path=None, **func_kwargs):
        ...
        self.logging("\teval_batch_size = {}".format(self.setting.eval_batch_size))
        ...
        #     # total_info = torch.Tensor(total_info)
        ...
        if dump_pkl_path is not None:
            default_dump_pkl(total_info, dump_pkl_path)

        return total_info

    def base_test(self, batch, get_info_on_batch):
        # prepare data loader
        if self.model_save:
            self.model.load_state_dict(torch.load(self.model_save_path))
        self.model.eval()
        batch_info = get_info_on_batch(self, batch)
        return batch_info
