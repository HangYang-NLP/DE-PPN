...
from DEE.ner_model_transformer import NERModel
from DEE.setpred4DEE import SetPred4DEE
...
def get_doc_span_info_list(doc_token_types_list, doc_fea_list, use_gold_span=False):
    ...
    for doc_token_types, doc_fea in zip(doc_token_types_list, doc_fea_list):
        ...
        gold_span_idx2pred_span_idx, pred_event_arg_idxs_objs_list, pred_event_type_idxs_list = doc_fea.generate_dag_info_for(span_token_tup_list, return_miss=True)
...
class SetPre4DEEModel(nn.Module):
    ...
    def __init__(self, config, event_type_fields_pairs, ner_model=None):
        ...
        if config.train_on_multi_roles == True:
            for event_type, role_type_list in self.event_type_fields_pairs:
